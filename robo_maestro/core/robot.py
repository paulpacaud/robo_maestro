...
class Robot:
    ...
    def gripper_move(self, target, low_force_mode=False, wait=True):  # OK
        ...
        time.sleep(
            6
        )  # the low-level controller does not wait for the gripper to reach the target position, so we need to wait here
        log_success(
            f"Gripper moved to position {target} with low force mode {low_force_mode}."
        )
    ...
    def _execute_gripper(self, state):
        log_info(f"Executing gripper command: {state}")
        if state == "open":
            self.open_gripper()
        else:
            self.close_gripper()
    ...
    def _plan_cartesian(self, planning_component, max_iterations=TRY_PLANNING_MAX_ITER):
        ...
        for iteration in range(max_iterations):
            log_info(
                f"Trying cartesian planning for {group_name}, iteration {iteration + 1}"
            )
            ...
            if valid_trajectory:
                log_success(
                    f"Cartesian planning succeeded for {group_name} after {iteration + 1} iterations"
                )
    ...
    def _plan_and_execute(
        self, robot, planning_component, cartesian_only, sleep_time=0.0
    ):
    ...
    def _limit_position(self, position):
        ...
        if not np.array_equal(position, new_position):
            log_warn(
                f"Safety _limit_position() function called and limited the pos to: {new_position}"
            )
        return new_position
    ...
    def go_to_pose(self, action, cartesian_only=False, sleep_time=3.0):
        ...
        goal.pose.position.x, goal.pose.position.y, goal.pose.position.z = map(
            float, target_pos
        )
        (
        ...
        ) = map(float, target_quat)
        self.left_arm.set_goal_state(
            pose_stamped_msg=goal, pose_link="left_gripper_grasp_frame"
        )
        success_arm = self._plan_and_execute(
            self.ur, self.left_arm, cartesian_only, sleep_time=sleep_time
        )

        # Gripper — only actuate if state changed
        target_grip_str = "open" if target_grip < 0.5 else "close"
