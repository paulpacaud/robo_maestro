...
class PolicyServer:
    ...
    def __init__(
        self, policy, host: str = "*", port: int = 5555, api_token: str = None
    ):
        self.policy = policy
        self.running = True
    ...
    @staticmethod
    def start_server(policy, host: str, port: int, api_token: str = None):
...
class PolicyClient:
    ...
    def get_action(
        self, batch: dict[str, Any], options: dict[str, Any] | None = None
    ) -> EasyDict:
        response = self.call_endpoint(
            "get_action", {"batch": batch, "options": options}
        )
        return EasyDict(response)
